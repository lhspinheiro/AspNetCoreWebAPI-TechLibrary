...
public class RegisterBooksValidator : AbstractValidator<RequestRegisterBooksJson>
{
    public RegisterBooksValidator()
    {
        ...
        RuleFor( request => request.Author).NotEmpty().WithMessage("Author is required");
        RuleFor( request => request.Amount).GreaterThan(0).WithMessage("Amount is required and must be greater than 0");
